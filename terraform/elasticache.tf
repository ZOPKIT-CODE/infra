...
# ---------------------------------------------------------------------------
# Security group: allow 6379 from the EKS data-plane (node SG) and from the
# cluster primary SG (managed-node/pod traffic). Egress open.
# ---------------------------------------------------------------------------
resource "aws_security_group" "valkey" {
  name        = "${local.name_prefix}-valkey"
  description = "Valkey ElastiCache access from EKS nodes/pods"
  vpc_id      = module.vpc.vpc_id

  ingress {
    description     = "Valkey from EKS node security group"
    from_port       = 6379
    to_port         = 6379
    protocol        = "tcp"
...
# ---------------------------------------------------------------------------
# Secrets Manager: connection details for the apps (consumed via IRSA/ESO).
# REDIS_URL uses the rediss:// (TLS) scheme + AUTH token + primary endpoint.
# ---------------------------------------------------------------------------
